...
#                     [5,5,self._channel,16], stddev=0.1, dtype=tf.float32))
#                 h_conv1 = tf.nn.conv2d(self._x, conv1_kernal, strides=[1,2,2,1], padding="SAME")
#                 conv1_b = tf.Variable(initial_value=tf.zeros([16], dtype=tf.float32))
#                 conv1 = tf.nn.leaky_relu(h_conv1 + conv1_b, alpha=0.1)
#             with tf.name_scope("conv2"):
#                 conv2_kernal = tf.Variable(initial_value=tf.truncated_normal(
#                     [5,5,16,32], stddev=0.1, dtype=tf.float32))
#                 h_conv2 = tf.nn.conv2d(conv1, conv2_kernal, strides=[1,2,2,1], padding="SAME")
#                 conv2_b = tf.Variable(initial_value=tf.zeros([32], dtype=tf.float32))
...
#                 conv3_b = tf.Variable(initial_value=tf.zeros([64], dtype=tf.float32))
...
#                     [5,5,64,128], stddev=0.1, dtype=tf.float32))
#                 h_conv4 = tf.nn.conv2d(conv3, conv4_kernal, strides=[1,2,2,1], padding="SAME")
#                 conv4_b = tf.Variable(initial_value=tf.zeros([128], dtype=tf.float32))
...
#                 upscale1_b = tf.Variable(initial_value=tf.zeros([256], dtype=tf.float32))
...
#                     [3,3,64,128], stddev=0.1, dtype=tf.float32))
#                 upscale2_b = tf.Variable(initial_value=tf.zeros([128], dtype=tf.float32))
...
#                 upscale3_b = tf.Variable(initial_value=tf.zeros([64], dtype=tf.float32))
...
#                     [3,3,16,32], stddev=0.1, dtype=tf.float32))
#                 upscale4_b = tf.Variable(initial_value=tf.zeros([32], dtype=tf.float32))
...
#                     [5, 5, 8, self._channel], stddev=0.1, dtype=tf.float32))
...
#                 upscale22_kernal = tf.Variable(initial_value=tf.truncated_normal(
#                     [3,3,64,128], stddev=0.1, dtype=tf.float32))
#                 upscale22_b = tf.Variable(initial_value=tf.zeros([128], dtype=tf.float32))
...
#                 upscale32_kernal = tf.Variable(initial_value=tf.truncated_normal(
#                     [3,3,32,64], stddev=0.1, dtype=tf.float32))
#                 upscale32_b = tf.Variable(initial_value=tf.zeros([64], dtype=tf.float32))
...
#                 upscale42_kernal = tf.Variable(initial_value=tf.truncated_normal(
#                     [3,3,16,32], stddev=0.1, dtype=tf.float32))
#                 upscale42_b = tf.Variable(initial_value=tf.zeros([32], dtype=tf.float32))
...
#                 conv52_kernal = tf.Variable(initial_value=tf.truncated_normal(
#                     [5, 5, 8, self._channel], stddev=0.1, dtype=tf.float32))
...
#         source = cv2.imread(r"F:\tensorflow\automodel\scrawler\video-1\trainImg\18.jpg") / 255
#         source = np.reshape(source, [1, 128, 128, 3])
...
class AutoEncoder:
    ...
    def define_network(self):
        ...
        def define_encoder():
            with tf.name_scope("conv1"):
                conv1_kernal = tf.Variable(initial_value=tf.truncated_normal(
                    [5,5,self._channel,64], stddev=0.1, dtype=tf.float32))
                h_conv1 = tf.nn.conv2d(self._x, conv1_kernal, strides=[1,2,2,1], padding="SAME")
            ...
            with tf.name_scope("conv4"):
                ...
                tmp_dim = int(flatten1.get_shape()[1])
            with tf.name_scope("dense1"):
                w1 = tf.Variable(initial_value=xavier_init(tmp_dim, 512))
                b1 = tf.Variable(initial_value=tf.zeros([512], dtype=tf.float32))
                link1 = tf.nn.relu(tf.nn.bias_add(tf.matmul(flatten1, w1), b1))
            with tf.name_scope("dense2"):
                w2 = tf.Variable(initial_value=xavier_init(512, tmp_dim))
                b2 = tf.Variable(initial_value=tf.zeros([tmp_dim], dtype=tf.float32))
        ...
        def define_decoder1():
            ...
            with tf.name_scope("conv51"):
                conv5_kernal = tf.Variable(initial_value=tf.truncated_normal(
                    [5, 5, 32, self._channel], stddev=0.1, dtype=tf.float32))
                h_conv5 = tf.nn.conv2d(upscale4, conv5_kernal, strides=[1, 1, 1, 1], padding="SAME")
                conv5_b = tf.Variable(initial_value=tf.zeros([self._channel], dtype=tf.float32))
                conv5 = tf.nn.sigmoid(tf.nn.bias_add(h_conv5, conv5_b))
                self._reconstruct1 = conv5
        ...
        for step in range(self._max_step):
            ...
            for time in range(sourceCount + destCount):
                ...
                print(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "- [ dest ] cost: %3.5f" % loss,
                      "avg: %.3f" % tmp, "time:", time)
            print(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "- source_avg_cost: %3.5f" % source_avg_cost,
                  "- dest_avg_cost: %10.3f" % dest_avg_cost, "step:", step)
            tf.train.Saver().save(self._sess, save_path="{0}encoder".format(self._modelPath), global_step=step)
        plt.show()
    def load_model(self):
        tf.train.Saver().restore(self._sess, tf.train.latest_checkpoint(self._modelPath))
...
if __name__ == "__main__":
    obj = AutoEncoder(5e-5, 100, 32, "F:/tensorflow/automodel/model/")
    # obj.load_model()
    # obj.generateImage()
    # obj.showAll()
